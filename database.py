...
# from sqlalchemy.dialects.postgresql import JSONB  # Commented out for SQLite compatibility
from sqlalchemy.orm import sessionmaker
from sqlalchemy.exc import SQLAlchemyError
import logging
...
class DatabaseManager:
    ...
    def _define_tables(self):
        self.campaigns_table = Table('campaigns', self.metadata,
            ...
            Column('buying_type', String),
            Column('spend_cap', String),
            Column('special_ad_categories', String),  # Changed from JSONB to String for SQLite
            Column('bid_strategy', String)
        )
        ...
        self.adsets_table = Table('adsets', self.metadata,
            ...
            Column('updated_time', TIMESTAMP),     # ⬅️ Changed
            Column('account_id', String),
            Column('targeting', String),  # Changed from JSONB to String for SQLite
            Column('pacing_type', String),  # Changed from JSONB to String for SQLite
            Column('attribution_spec', String),  # Changed from JSONB to String for SQLite
            Column('bid_strategy', String),
            Column('promoted_object', String),  # Changed from JSONB to String for SQLite
            Column('frequency_cap', String),
            Column('frequency_control_specs', String)  # Changed from JSONB to String for SQLite
        )
        ...
        self.ads_table = Table('ads', self.metadata,
            ...
            Column('attribution_setting', String),
            Column('ad_creative_id', String),
            Column('creative', String)  # Changed from JSONB to String for SQLite
        )

        self.ad_insights_table = Table('ad_insights', self.metadata,
            Column('id', String, primary_key=True),  # Primary key
            Column('ad_id', String),
            Column('campaign_id', String),
            ...
            Column('ctr', Float),
            Column('cpp', Float),
            Column('conversions', Integer),        # ⬅️ Added missing column
            Column('conversion_values', String)  # Changed from JSONB to String for SQLite
        )

        self.campaign_insights_table = Table('campaign_insights', self.metadata,
            Column('id', String, primary_key=True),  # ⬅️ Added primary key
            Column('campaign_id', String),
            Column('date_start', DATE),            # ⬅️ Changed
            Column('date_stop', DATE),             # ⬅️ Changed
            Column('spend', Float),
    ...
    def _bulk_insert(self, table, records):
        try:
            ...
            for i, record in enumerate(records):
                ...
                # Generate IDs for insights tables if missing
                if table.name in ['ad_insights', 'campaign_insights'] and not cleaned_record.get('id'):
                    cleaned_record['id'] = f"{table.name}_{i}_{record.get('ad_id', record.get('campaign_id', 'unknown'))}"
                
                # Convert date strings to datetime objects for SQLite
                for col in ['created_time', 'updated_time', 'start_time', 'stop_time', 'end_time', 'date_start', 'date_stop']:
                    if col in cleaned_record and cleaned_record[col]:
                        try:
                            # Handle ISO format dates from Facebook API
                            date_str = cleaned_record[col]
                            if isinstance(date_str, str):
                                ...
                                cleaned_record[col] = datetime.fromisoformat(date_str.replace('T', ' '))
                        except Exception as e:
                            logger.warning(f"Could not parse date {col}: {cleaned_record[col]}, error: {e}")
                            cleaned_record[col] = None
                
                # Convert lists and dicts to JSON strings for SQLite
                for col in ['special_ad_categories', 'targeting', 'pacing_type', 'attribution_spec', 'promoted_object', 'frequency_control_specs', 'creative', 'conversion_values']:
                    if col in cleaned_record:
                        if isinstance(cleaned_record[col], (list, dict)):
                            import json
                            cleaned_record[col] = json.dumps(cleaned_record[col])
                        elif cleaned_record[col] == []:
                            cleaned_record[col] = '[]'
                        elif cleaned_record[col] == {}:
                            cleaned_record[col] = '{}'
